...
const INSTRUCTIONS = [
    ...
    'Long operations are jobs: convert/build_lod/render_image/generate_collision/trim_region/get_summary return {jobId}; poll with jobs(action="wait") then jobs(action="get"). Jobs queue FIFO and run up to a concurrency cap at once (default 1), so a new job may report status="queued" first.',
