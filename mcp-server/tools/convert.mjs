...
export function register(server) {
    ...
    server.registerTool('build_lod', {
        ...
        inputSchema: {
            ...
            decimateAlgorithm: z.enum(['adaptive', 'uniform']).optional().describe('decimate: algorithm for every derived level — "adaptive" (--decimate, default) allocates removal by local error; "uniform" (--decimate-uniform) removes at a flat rate. Ignored in combine mode. "uniform" is the only algorithm that runs on CPU once a scene is large enough to split into multiple blocks; adaptive with device "cpu" there fails as gpu-required.'),
