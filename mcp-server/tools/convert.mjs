...
export function register(server) {
    server.registerTool('convert', {
        ...
        inputSchema: {
            ...
            decimateMode: z.enum(['adaptive', 'uniform']).optional().describe('Decimation algorithm (default adaptive). "adaptive" (--decimate) allocates removal by local error — best on mixed-scale scenes; "uniform" (--decimate-uniform) thins evenly, uses about half the memory, and is the only one that runs on CPU for large scenes. Only used with decimate.'),
    ...
    server.registerTool('build_lod', {
        ...
        inputSchema: {
            ...
            decimateMode: z.enum(['adaptive', 'uniform']).optional().describe('decimate: algorithm for the per-level pre-decimation (default adaptive). "uniform" (--decimate-uniform) halves memory and is the only one that runs on CPU for large scenes.'),
