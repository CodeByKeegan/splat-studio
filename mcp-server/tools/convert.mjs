...
export function register(server) {
    server.registerTool('convert', {
        ...
        inputSchema: {
            ...
            decimateAlgorithm: z.enum(['adaptive', 'uniform']).optional().describe('Decimation algorithm (default uniform). "uniform" (--decimate) removes at a flat rate — lower memory; "adaptive" (--decimate-adaptive) allocates removal by local error — better on mixed-scale content such as skies. Only used with decimate. "uniform" is the only algorithm that runs on CPU once a scene is large enough to split into multiple blocks; adaptive with device "cpu" there fails as gpu-required.'),
    ...
    server.registerTool('build_lod', {
        ...
        inputSchema: {
            ...
            decimateAlgorithm: z.enum(['adaptive', 'uniform']).optional().describe('decimate: algorithm for every derived level — "uniform" (--decimate, default) removes at a flat rate; "adaptive" (--decimate-adaptive) allocates removal by local error. Ignored in combine mode. "uniform" is the only algorithm that runs on CPU once a scene is large enough to split into multiple blocks; adaptive with device "cpu" there fails as gpu-required.'),
