...
export function register(server) {
    server.registerTool('convert', {
        ...
        inputSchema: {
            ...
            decimateAlgorithm: z.enum(['adaptive', 'uniform']).optional().describe('Decimation algorithm (default adaptive). "adaptive" (--decimate) allocates removal by local error — better on mixed-scale content such as skies; "uniform" (--decimate-uniform) removes at a flat rate — lower memory. Only used with decimate.'),
    ...
    server.registerTool('build_lod', {
        ...
        inputSchema: {
            ...
            decimateAlgorithm: z.enum(['adaptive', 'uniform']).optional().describe('decimate: algorithm for every derived level — "adaptive" (--decimate, default) allocates removal by local error; "uniform" (--decimate-uniform) removes at a flat rate. Ignored in combine mode.'),
