...
Planning, design, and the automated routines all live in the open, where they can be read
and changed by pull request. Nothing that decides what this project builds sits in a
private tool.

| What | Where |
| --- | --- |
| Backlog & design discussion | [GitHub issues](https://github.com/CodeByKeegan/splat-studio/issues) — `next` is the queue the dev routine pulls from |
| CLI flag coverage | [`docs/CLI_COVERAGE.md`](docs/CLI_COVERAGE.md), generated by `npm run coverage` |
| Direction | [`ROADMAP.md`](ROADMAP.md) |
| Scheduled agent routines | [`.agents/workflows/`](.agents/workflows/README.md) — the actual prompts |
| Reusable procedures | `.claude/skills/` (mirrored to `.agents/skills/`) |

When you design a feature, write the design where others can argue with it: an issue
comment or the PR body, not a scratch doc. See the `splat-studio-design-pass` skill.
...
- `npm run coverage` regenerates the CLI coverage table; `npm run check-routines`
  validates the routine bank + skill mirror. Both are cheap — run them after touching
  flags, skills, or routines.
...
- `splat-studio-design-pass` — design a control before building it (discover → design → critique)
...
The scheduled routines that call these skills are prompt files in
[`.agents/workflows/`](.agents/workflows/README.md) — `dep-update`, `pr-merge`, `dev-cycle`.
The scheduler only holds a pointer to them, so what runs each day is whatever is on `dev`:
change a routine by opening a PR against its file.
