...
Pass `decimateAlgorithm: "adaptive"` to allocate removal by local error, which is better
on mixed-scale content such as skies; omitted or `"uniform"` decimates every derived
level at a flat rate (lower memory). The effective choice lands in `build-meta.json`.
