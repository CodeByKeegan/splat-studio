...
// ----- generator input controls (.mjs sources) -----
// Keys off the selected INPUT, not the output format. If the generator advertises
// a `params` schema we render live sliders; otherwise a freeform key=val field. A
// "Generate & view" button (and slider release) generates to PLY and loads it
...
const updateGenParamsRow = async (): Promise<void> => {
    const input = convertInput.value;
    const isMjs = input.endsWith('.mjs');
...
convertInput.onchange = () => void updateGenParamsRow();

const doGenerateView = (): void => {
    const input = convertInput.value;
    if (!input.endsWith('.mjs')) { showToast('Pick a .mjs generator as the Convert input', true); return; }
