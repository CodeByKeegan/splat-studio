...
try {
    ...
    // 'uniform' switches to the CLI's own 3.3+ default flat-rate --decimate algorithm
    ...
    await check('LOD decimate mode: decimateAlgorithm uniform emits --decimate on each pre-command', async () => {
        ...
        assert(lines.slice(0, 2).every((l) => l.includes('--decimate ') && !l.includes('--decimate-adaptive')), `pre-commands missing --decimate: ${job.command}`);
