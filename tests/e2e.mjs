...
try {
    ...
    await check('streamed LOD (decimate, 3 levels) bakes + cleans temps', async () => {
        ...
        // no decimateAlgorithm -> adaptive: every pre-command emits --decimate-adaptive
    ...
    // decimation strategy: adaptive (default/omitted) emits --decimate-adaptive;
    // 'uniform' emits plain --decimate (the CLI's own default since 3.3)
    await check('decimate defaults to adaptive (--decimate-adaptive, not plain --decimate)', async () => {
        ...
        assert(!job.command.includes('--decimate 50%'), `unexpected plain --decimate: ${job.command}`);
    });
