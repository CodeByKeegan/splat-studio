...
try {
    ...
    await check('streamed LOD (decimate, 3 levels) bakes + cleans temps', async () => {
        ...
        assert(!/(^|\s)--decimate(\s|$)/.test(job.command), `LOD default emitted uniform: ${job.command}`);
    });
    ...
    // 'uniform' uses the plain --decimate flat-rate algorithm (the CLI's own default
    // as of splat-transform 3.3.0, though our app-level default stays adaptive)
    await check('decimate defaults to adaptive (--decimate-adaptive, not plain --decimate)', async () => {
        const job = await runJob('/api/convert', {
        ...
        assert(!/(^|\s)--decimate(\s|$)/.test(job.command), `unexpected plain --decimate: ${job.command}`);
    });
    ...
    await check('LOD decimate mode: decimateAlgorithm uniform emits plain --decimate on each pre-command', async () => {
        ...
        assert(lines.slice(0, 2).every((l) => /(^|\s)--decimate(\s|$)/.test(l)), `pre-commands missing plain --decimate: ${job.command}`);
