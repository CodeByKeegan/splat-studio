...
// poll until the job settles (done/error/cancelled); returns the finished job
...
try {
    ...
    await check('streamed LOD (decimate, 3 levels) bakes + cleans temps', async () => {
        ...
        // no decimateAlgorithm -> uniform: every pre-command keeps plain --decimate
        const pre = job.command.split('\n').slice(0, 2);
        assert(pre.every((l) => l.includes('--decimate ')), `LOD default not uniform: ${job.command}`);
        assert(!job.command.includes('--decimate-adaptive'), `LOD default emitted adaptive: ${job.command}`);
    });
    ...
    await check('build-meta.json records the decimate recipe + per-level gaussians', async () => {
        ...
        assert(s.decimateAlgorithm === 'uniform', `settings decimateAlgorithm: ${s.decimateAlgorithm}`);
    });
    ...
    // decimation strategy: uniform (default/omitted, reverted in 3.3) stays plain
    // --decimate; 'adaptive' switches to the --decimate-adaptive algorithm
    await check('decimate defaults to uniform (--decimate, not --decimate-adaptive)', async () => {
        const job = await runJob('/api/convert', {
    ...
    await check('decimateAlgorithm adaptive emits --decimate-adaptive', async () => {
        const job = await runJob('/api/convert', {
            input: 'demo-room.ply', format: 'ply',
            options: { decimate: '50%', decimateAlgorithm: 'adaptive', device: SKIP_GPU ? 'cpu' : 'auto' }
        });
        assert(job.status === 'done', `job ${job.status}: ${(job.log || '').slice(-200)}`);
        assert(job.command.includes('--decimate-adaptive 50%'), `no --decimate-adaptive 50% in cmd: ${job.command}`);
    });

    await check('LOD decimate mode: decimateAlgorithm adaptive emits --decimate-adaptive on each pre-command', async () => {
        const job = await runJob('/api/convert', {
            input: 'demo-room.ply', format: 'lod',
            options: { device: SKIP_GPU ? 'cpu' : 'auto', lodLevels: 3, lodKeepPercent: 50, decimateAlgorithm: 'adaptive' }
        });
        assert(job.status === 'done', `job ${job.status}: ${(job.log || '').slice(-200)}`);
        const lines = job.command.split('\n');
        assert(lines.slice(0, 2).every((l) => l.includes('--decimate-adaptive')), `pre-commands missing --decimate-adaptive: ${job.command}`);
        ...
        assert(bm.settings.decimateAlgorithm === 'adaptive', `recipe decimateAlgorithm: ${bm.settings.decimateAlgorithm}`);
    });
    ...
    await check('cancel a queued job before it ever runs', async () => {
        ...
        const jb = await waitJob(b);
        assert(jb.status === 'cancelled' && jb.startedAt === null, `queued cancel -> ${jb.status}, startedAt ${jb.startedAt}`);
