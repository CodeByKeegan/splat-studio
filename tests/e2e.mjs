...
try {
    ...
    // 'uniform' switches to plain --decimate (the flat-rate algorithm, default since 3.3)
    ...
    await check('LOD decimate mode: decimateAlgorithm uniform emits plain --decimate on each pre-command', async () => {
        ...
        assert(lines.slice(0, 2).every((l) => l.includes('--decimate ') && !l.includes('--decimate-adaptive')),
            `pre-commands missing plain --decimate: ${job.command}`);
