...
try {
    ...
    await check('build_lod (decimate, CPU) -> lod-meta.json output', async () => {
        const j = data(await call('build_lod', { project: 'Demo', input: 'demo-room.ply', mode: 'decimate', device: 'cpu', lodLevels: 2, lodKeepPercent: 50, decimateAlgorithm: 'adaptive' }));
        assert(j.jobId, `no jobId: ${JSON.stringify(j)}`);
        ...
        // field would be stripped by the SDK and fall back to uniform
        const full = data(await call('jobs', { action: 'get', id: j.jobId }));
        assert(full.command?.includes('--decimate-adaptive'), `no --decimate-adaptive in cmd: ${full.command}`);
    });

    // the decimation-algorithm choice must survive MCP schema validation: the SDK
    // strips undeclared args, so an unwired field would silently fall back to uniform
    await check('convert carries decimateAlgorithm through to --decimate-adaptive', async () => {
        const j = data(await call('convert', { project: 'Demo', input: 'demo-room.ply', format: 'ply', decimate: '50%', decimateAlgorithm: 'adaptive', device: 'cpu' }));
        assert(j.jobId, `no jobId: ${JSON.stringify(j)}`);
        const w = data(await call('jobs', { action: 'wait', id: j.jobId, timeout_ms: 120000 }));
        assert(w.status === 'done', `decimate job: ${JSON.stringify(w).slice(0, 160)}`);
        const full = data(await call('jobs', { action: 'get', id: j.jobId }));
        assert(full.command?.includes('--decimate-adaptive 50%'), `no --decimate-adaptive 50% in cmd: ${full.command}`);
    });
