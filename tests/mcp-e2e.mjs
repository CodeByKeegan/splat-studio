...
try {
    ...
    await check('build_lod (decimate, CPU) -> lod-meta.json output', async () => {
        ...
        const full = data(await call('jobs', { action: 'get', id: j.jobId }));
        assert(full.command?.includes('--decimate '), `no --decimate in cmd: ${full.command}`);
    });
    ...
    await check('convert carries decimateAlgorithm through to --decimate', async () => {
        ...
        const full = data(await call('jobs', { action: 'get', id: j.jobId }));
        assert(full.command?.includes('--decimate 50%'), `no --decimate 50% in cmd: ${full.command}`);
        assert(!full.command?.includes('--decimate-adaptive'), `unexpected --decimate-adaptive: ${full.command}`);
    });
